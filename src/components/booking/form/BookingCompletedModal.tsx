...
const BookingCompletedModal = ({ bookingRef, onCancel }: BookingCompletedModalProps) => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 w-full h-full flex items-center justify-center bg-black/50 z-50 p-4">
      <div className="w-full max-w-md bg-white rounded-lg p-8 shadow-2xl text-center animate-in fade-in zoom-in duration-300">
        ...
        <div className="mb-4 bg-gray-50 p-6 rounded-lg border-2 border-gray-200">
          ...
          <div className="relative">
            <p className="text-4xl sm:text-5xl font-extrabold text-red-600 mb-4">{bookingRef}</p>
            <button 
              onClick={copyToClipboard} 
              className="absolute top-0 right-0 p-2 text-gray-500 hover:text-gray-700 transition-colors"
              aria-label="Copy reference number"
            >
              {copied ? <Star className="h-5 w-5 text-green-500" /> : <Copy className="h-5 w-5" />}
            </button>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="text-sm text-gray-500">
            You will receive a confirmation email shortly with booking details.
          </div>
        </div>
      </div>
    </div>
