-- Fix MemberMST primary key issue by handling foreign key dependencies

-- Step 1: Drop the foreign key constraint from wishlist table that depends on MemberMST
ALTER TABLE public.wishlist DROP CONSTRAINT IF EXISTS wishlist_user_id_fkey;

-- Step 2: Drop the existing primary key constraint on MemberMST
ALTER TABLE public."MemberMST" DROP CONSTRAINT IF EXISTS membermst_pkey;

-- Step 3: Add uuid column temporarily and populate it
ALTER TABLE public."MemberMST" ADD COLUMN IF NOT EXISTS uuid_temp UUID DEFAULT gen_random_uuid();

-- Step 4: Update existing records to have uuid values in uuid_temp
UPDATE public."MemberMST" SET uuid_temp = gen_random_uuid() WHERE uuid_temp IS NULL;

-- Step 5: Update wishlist table to use the new uuid values before dropping the old id column
...
-- Step 6: Drop the old columns and rename the new ones
...
-- Step 7: Set the new uuid id column as primary key
ALTER TABLE public."MemberMST" ADD PRIMARY KEY (id);

-- Step 8: Recreate the foreign key constraint with the new uuid column
ALTER TABLE public.wishlist 
ADD CONSTRAINT wishlist_user_id_fkey 
FOREIGN KEY (user_id) REFERENCES public."MemberMST"(id) ON DELETE CASCADE;

-- Step 9: Add missing columns to MemberMST for Supabase compatibility (if they don't exist)
ALTER TABLE public."MemberMST" 
ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
ADD COLUMN IF NOT EXISTS original_phone TEXT;

-- Step 10: Update existing records to populate new timestamp columns
UPDATE public."MemberMST" 
SET created_at = COALESCE(created_at, now()), 
    updated_at = COALESCE(updated_at, now()) 
WHERE created_at IS NULL OR updated_at IS NULL;

-- Step 11: Create indexes for better performance (if they don't exist)
...
-- Step 12: Enable RLS on MemberMST
ALTER TABLE public."MemberMST" ENABLE ROW LEVEL SECURITY;

-- Step 13: Drop any existing policies to avoid conflicts
